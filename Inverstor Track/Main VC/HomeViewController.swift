...
// MARK:- creating SubHeader views
    ...
    func prepareBottomBar(){
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
            ...
            self.subHeaderBottomBar.frame = CGRect(x: initXValue, y: self.category.frame.maxY-5, width: (self.view.frame.size.width/3)/1.5, height: 4)
            self.subHeaderBottomBar.backgroundColor = UIColor(hexFromString: "4e4fcf")
            self.subHeaderBottomBar.layer.cornerRadius = 1
            self.subHeaderView.addSubview(self.subHeaderBottomBar)
            self.didTappedCategory()
        }
    }

// MARK:- Tapping on Category with Table view without XIB
    @objc func didTappedCategory(){
        let initx = self.category.frame.minX
        let initW = self.category.frame.size.width/6
        UIView.animate(withDuration: 0.3) {
            self.category.setTitleColor(UIColor(hexFromString: "#4e4fcf"), for: .normal)
            self.trending.titleLabel?.textColor = UIColor.black
            self.themes.titleLabel?.textColor = UIColor.black
        ...
        removeChildVCCompletionHandler { (isFinished) in
            if isFinished == true {
                self.addChildViewControllerWithView(CategoryViewController(), topConstant: topValue, bottomConstant: 0, leadingConstant: 0, trailingConstant: 0, container: self.containerView)
            }
        }
...
// MARK:- Tapping on Themes with Collection view
    @objc func didTappedThemes(){
        let initx = self.themes.frame.minX
        let initW = self.themes.frame.size.width/6
        UIView.animate(withDuration: 0.3) {
            self.themes.setTitleColor(UIColor(hexFromString: "#4e4fcf"), for: .normal)
            self.trending.titleLabel?.textColor = UIColor.black
            self.category.titleLabel?.textColor = UIColor.black
            self.subHeaderBottomBar.frame.origin.x = (initx+initW)
        }
        let topValue = self.subHeaderView.frame.maxY + 20
        removeChildVCCompletionHandler { (isFinished) in
            if isFinished == true {
                self.addChildViewControllerWithView(ThemesViewController(), topConstant: topValue , bottomConstant: 0, leadingConstant: 0, trailingConstant: 0, container: self.containerView)
            }
        }
        
    }
    
// MARK:- Tapping on Trending with Table view with XIB as custom cell
    @objc func didTappedTrending(){
        let initx = self.trending.frame.minX
        let initW = self.trending.frame.size.width/6
        UIView.animate(withDuration: 0.3) {
            self.trending.setTitleColor(UIColor(hexFromString: "#4e4fcf"), for: .normal)
            self.themes.titleLabel?.textColor = UIColor.black
            self.category.titleLabel?.textColor = UIColor.black
            self.subHeaderBottomBar.frame.origin.x = (initx+initW)
        }
        ...
        removeChildVCCompletionHandler { (isFinished) in
            if isFinished == true {
                self.addChildViewControllerWithView(TrendingViewController(), topConstant: topValue , bottomConstant: 0, leadingConstant: 0, trailingConstant: 0, container: self.containerView)
            }
        }
    }
...
// MARK:- Extension Viewcontroller to Create Child VC
